...
if(isset($_POST['submit'])){
    $assignment_id = htmlentities(isset($_POST["assignment_id"]) ? trim($_POST["assignment_id"]): "");
    ...
    $fields_required = array("assignment_id", "course_code", "date", "weight");
 ...
 $num_assignment = htmlentities(isset($_POST["count"]) ? trim($_POST["count"]): "");
    if($num_assignment > 0){
     ...
     if(empty($errors)){
        global $connection;
        for($i=0; $i<=$j-1; $i++){
             $query = "INSERT INTO assignment(assignment_id, course_code, due_date, assignment_itself, option_a, option_b, option_c, option_d, answer) VALUES({$assignment_id}, '{$course_code}', '{$due_date}', '{$questions[$i]}', '{$Aoptions[$i]}', '{$Boptions[$i]}','{$Coptions[$i]}','{$Doptions[$i]}', '{$answers[$i]}')";
             $assignment = mysqli_query($connection, $query);
            confirm_query($assignment);
        }
        $query = "INSERT INTO activity_notifications(course_code, notif_type, act_id, due_date) VALUES('{$course_code}','Assignment', {$assignment_id}, '{$due_date}')";
         $notif = mysqli_query($connection, $query);
         confirm_query($notif);
         $query = "INSERT INTO assignment_collection(assignment_id, course_code, weight) VALUES({$assignment_id}, '{$course_code}',{$weight})";
        ...
        if(empty($errors)){
             move_uploaded_file($_FILES['file']['tmp_name'],"../../uploads/assignments/".$_FILES['file']['name']);
        $query = "INSERT INTO assignment(assignment_id, course_code,due_date, assignment_upload) VALUES ({$assignment_id},'{$course_code}','{$due_date}','".$_FILES['file']['name']. "')";
        $upload_assignment = mysqli_query($connection, $query);
        confirm_query($upload_assignment);
        }
        $query = "INSERT INTO activity_notifications(course_code, notif_type, act_id, due_date) VALUES('{$course_code}','Assignment', {$assignment_id}, '{$due_date}')";
         $notif = mysqli_query($connection, $query);
         confirm_query($notif);
            ...
            <div id="page-wrapper">
                ...
                <div id="title-breadcrumb-option-demo" class="page-title-breadcrumb">
                    ...
                    <ol class="breadcrumb page-breadcrumb pull-right">
                        ...
                        <li class="hidden"><a href="#">Assignments</a>&nbsp;&nbsp;<i class="fa fa-angle-right"></i>&nbsp;&nbsp;</li>
                    ...
                    <div id="tab-general">
                            <form method="post" action="" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <input type="text" name="assignment_id" placeholder="Assignment ID" class="form-control">
                                </div>
                                <div class="form-group">
                                     <input type="text" name="weight" placeholder="Assignment Weight" class="form-control">
                                </div>
                                 <div class="form-group">
                                    <input type="text" name="course_code" value="<?php echo $_GET['course_code']; ?>" class="form-control">
                                </div>
                                 <div class="form-group">
                                    <input type="date" name="date" placeholder="Due Date" class="form-control">
                                </div>
