...
if(isset($_POST['submit'])){
    ...
    $fields_required = array( "course_code");
    ...
    if($num_assignment > 0){
     ...
     if(empty($errors)){
        global $connection;
        for($i=0; $i<=$j-1; $i++){
             $query = "INSERT INTO quizup_questions(course_code,  question, answer, option_a, option_b, option_c, option_d) VALUES( '{$course_code}', '{$questions[$i]}', '{$answers[$i]}', '{$Aoptions[$i]}', '{$Boptions[$i]}', '{$Coptions[$i]}', '{$Doptions[$i]}')";
