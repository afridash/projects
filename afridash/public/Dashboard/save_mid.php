...
<?php 
                        ...
                        $query = "SELECT course_id, course_semester FROM courses WHERE course_code = '{$course_code}' ";
                        ...
                        while($new_forum_students = mysqli_fetch_assoc($get_forum_students)){
                            ...
                            while($new_member = mysqli_fetch_assoc($get_new_member)){
                                    ...
                                    if(isset($_POST[$string]) && !empty($_POST[$string])){
                                        ...
                                        $query = "SELECT grade FROM mid_term WHERE user_id = {$new_member['user_id']} AND course_code = '{$course_code}' LIMIT 1";
                                        ...
                                        $query = "INSERT INTO mid_term(user_id, course_code, grade) VALUES({$new_member['user_id']}, '{$course_code}', {$assign_score})";
                                        $store_assignment=mysqli_query($connection, $query);
                                        confirm_query($store_assignment);
                                        }else{
                                            $query = "UPDATE mid_term SET grade = {$assign_score} WHERE user_id = {$new_member['user_id']} AND course_code = '{$course_code}' ";
