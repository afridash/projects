...
if(isset($_POST['submit'])){
    $test_id = htmlentities(isset($_POST["test_id"]) ? trim($_POST["test_id"]): "");
    ...
    $fields_required = array("test_id", "course_code", "date");
    ...
    if($num_assignment > 0){
     ...
     if(empty($errors)){
        global $connection;
        for($i=0; $i<=$j-1; $i++){
             $query = "INSERT INTO test_questions(course_code, test_id, question, answer, option_a, option_b, option_c, option_d, due_date) VALUES( '{$course_code}',{$test_id}, '{$questions[$i]}', '{$answers[$i]}', '{$Aoptions[$i]}', '{$Boptions[$i]}', '{$Coptions[$i]}', '{$Doptions[$i]}', '{$due_date}')";
             $assignment = mysqli_query($connection, $query);
            confirm_query($assignment);
        }
         $query = "INSERT INTO activity_notifications(course_code, notif_type,act_id, due_date) VALUES('{$course_code}','Test',{$test_id}, '{$due_date}')";
         $notif = mysqli_query($connection, $query);
         confirm_query($notif);
         $query ="INSERT INTO test_collection(course_code, test_id, weight) VALUES ('{$course_code}', {$test_id}, {$weight})";
        ...
        if(empty($errors)){
             move_uploaded_file($_FILES['file']['tmp_name'],"../../uploads/tests/".$_FILES['file']['name']);
        $query = "INSERT INTO test_questions(course_code,test_id,due_date, test_upload) VALUES ('{$course_code}',{$test_id},'{$due_date}','".$_FILES['file']['name']. "')";
        $upload_test = mysqli_query($connection, $query);
        confirm_query($upload_test);
        }
        $query = "INSERT INTO activity_notifications(course_code, notif_type,act_id, due_date) VALUES('{$course_code}','Test',{$test_id}, '{$due_date}')";
         $notif = mysqli_query($connection, $query);
         confirm_query($notif);
            ...
            <div id="page-wrapper">
                ...
                <div id="title-breadcrumb-option-demo" class="page-title-breadcrumb">
                    ...
                    <ol class="breadcrumb page-breadcrumb pull-right">
                        ...
                        <li class="hidden"><a href="#">Test</a>&nbsp;&nbsp;<i class="fa fa-angle-right"></i>&nbsp;&nbsp;</li>
                    ...
                    <div id="tab-general">
                            <form method="post" action="" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <input type="text" name="test_id" placeholder="Test ID" class="form-control">
                                </div>
                                <div class="form-group">
                                     <input type="text" name="weight" placeholder="Test Weight" class="form-control">
                                </div>
                                 <div class="form-group">
