...
$score = 0;
$ans = array();
$query = "SELECT answer FROM test_questions WHERE test_id = {$_GET['id']} AND course_code = '{$_GET['course_code']}' ";
...
$query = "INSERT INTO test_scores (test_id, course_code, user_id, score) VALUES({$_GET['id']},'{$_GET['course_code']}',{$_SESSION['user_id']}, {$total})";
...
<div id="contentOneTwo"></div>
                ...
                <div id="title-breadcrumb-option-demo" class="page-title-breadcrumb">
                    <div class="page-header pull-left">
                        <div class="page-title">
                            Take Test <?php echo $_GET['id']; ?></div>
                    </div>
     ...
     <form method="post" action="">
        ...
        <?php 
        global $connection;
        $query = "SELECT * FROM test_questions WHERE course_code = '{$_GET['course_code']}' AND test_id= '{$_GET['id']}' ";
        $examples  = mysqli_query($connection, $query);
        $count = 0;
        while($example = mysqli_fetch_assoc($examples)){
            if($example['test_upload']==""){
            $count +=1;
            ?>
        <ul class="questions">
        ...
        <?php }else{ ?>
         <iframe src="<?php load_test($_GET['course_code'], $_GET['id']); ?>" width="100%" height="600px" ></iframe>     
         <?php
            }
        }
        ?>
